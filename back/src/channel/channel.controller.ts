...
export class ChannelsController {
  ...
  async join(@Req() req, @Body() data: any) {
    console.log(
      "-------------------------- Starting Joining a Channel  -------------------------- "
    );
    console.log(data);
    if (data) {
      if (
        !data.sendData.id_channel ||
        !data.sendData.name ||
        !data.sendData.visibility
      ) {
        return false;
      }
  ...
  async updatePass(@Req() req, @Body() data: any) {
    ...
    try {
      ...
      if (user) {
        const updated = await this.channelsService.updatePass(
          data,
          user.id_user
        );
  ...
  async removePass(@Req() req, @Body() data: any) {
    ...
    if (data) {
      if (!data.id_channel || !data.user_id) return false;
    } else return false;
    try {
      const decode = this.jwt.verify(req.cookies[this.config.get("cookie")]);
      const user = await this.UsersService.findById(decode.id);
      ...
      if (user) {
        const remove = await this.channelsService.removePass(
          data,
          user.id_user
        );
  ...
  async getAllChannels(@Req() req, @Body() data: any) {
    ...
    try {
      const decode = this.jwt.verify(req.cookies[this.config.get("cookie")]);
      const user = await this.UsersService.findById(decode.id);
      const myAllChannels = await this.channelsService.getAllChannels(
        user.id_user
      );
      ...
      if (myAllChannels) {
        ...
        for (const channels of myAllChannels) {
          message = "";
          sent = null;
          const lastMsg = await this.channelsService.getTheLastMessageOfChannel(
            channels.channelId
          );
          if (lastMsg) {
          ...
          const admins = await this.channelsService.getAllAdmins(
            channels.channelId
          );
          const memebers = await this.channelsService.getAllMembers(
            channels.channelId
          );
          const owners = await this.channelsService.getAllOwners(
            channels.channelId
          );
          const newCh = {
